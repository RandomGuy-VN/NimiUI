--[[
    ...
    API:
      ...
      Sec:Button{ Title=, Desc=, Callback= }
      Sec:Input{ Title=, Desc=, Value=, Placeholder=, Type="Textarea", Callback= }
      Sec:Slider{ Title=, Value={Min=,Max=,Default=}, Step=, Callback= }
      Sec:Dropdown{ Title=, Desc=, Values={...}, Value=, Multi=false, Callback= }
...
----------------------------------------------------------------
-- ICONS — Material set ported verbatim from Luna-Interface-Suite
-- (credit: Latte Softworks for Lucide, qweery for Material).
-- Lucide is fetched lazily via HttpGet from latte-soft/lucide-roblox
-- and cached. GetIcon(name, source) returns:
--   * "rbxassetid://N" / "http://...asset?id=N" string  (Material/Custom)
--   * { id = N, imageRectSize = Vector2, imageRectOffset = Vector2 }  (Lucide)
--   * nil if not found
-- iconImage(parent, name, source, props) creates an ImageLabel using GetIcon,
-- falling back to a TextLabel symbol if the asset can't be resolved.
----------------------------------------------------------------
...
-- Unicode fallback symbols for when an icon name isn't in any asset set.
-- (Used by iconImage's fallback path and by GetIconSymbol.)
...
local _lucideCache    -- table once Lucide is loaded
local _lucideTried    = false

local function _loadLucide()
    if _lucideCache or _lucideTried then return _lucideCache end
    _lucideTried = true
    ...
    local ok2, mod = pcall(function() return loadstring(data)() end)
    if ok2 then _lucideCache = mod end
    return _lucideCache
end

-- Public: resolve an icon name to either an asset id string or a Lucide struct.
local function GetIcon(icon, source)
    ...
    if source == "Lucide" then
        local icons = _loadLucide()
        if not icons then return nil end
        local name = string.match(string.lower(tostring(icon)), "^%s*(.*)%s*$")
    ...
    -- Material / any other named set on IconModule
    local set = IconModule[source]
    if set and set[icon] then return set[icon] end

    -- Implicit fallback: caller passed a name without a source — try Material
...
NimiUI.GetIcon = GetIcon
NimiUI.IconModule = IconModule

-- Returns a unicode fallback symbol for a name, or "•" if unknown.
local function GetIconSymbol(name)
    if not name then return "" end
    return FALLBACK_SYMBOLS[name] or "•"
end

-- Backward-compat shim for earlier code that called iconText.
local function iconText(name) return GetIconSymbol(name) end

-- Creates an ImageLabel for the icon if resolvable, otherwise a TextLabel
-- with a unicode symbol. Caller passes desired props (Size, Position,
-- AnchorPoint, BackgroundTransparency, ImageColor3, TextColor3, etc.).
-- `source` defaults to "Material". Pass "Lucide" to use the spritesheet set.
local function iconImage(parent, name, source, props)
    props = props or {}
    local resolved = GetIcon(name, source)

    if resolved == nil then
        -- Text fallback
        local lbl = Instance.new("TextLabel")
        lbl.BackgroundTransparency = props.BackgroundTransparency or 1
        lbl.Font       = props.Font       or FONT_BOLD
        lbl.Text       = GetIconSymbol(name)
        lbl.TextColor3 = props.TextColor3 or props.ImageColor3 or Color3.fromRGB(255,255,255)
        lbl.TextSize   = props.TextSize   or 16
        lbl.TextXAlignment = Enum.TextXAlignment.Center
        lbl.TextYAlignment = Enum.TextYAlignment.Center
        if props.Size         then lbl.Size         = props.Size end
        if props.Position     then lbl.Position     = props.Position end
        if props.AnchorPoint  then lbl.AnchorPoint  = props.AnchorPoint end
        if props.LayoutOrder  then lbl.LayoutOrder  = props.LayoutOrder end
    ...
    img.BackgroundTransparency = props.BackgroundTransparency or 1
    img.BorderSizePixel = 0
    if props.ImageColor3 then img.ImageColor3 = props.ImageColor3 end
    if props.Size         then img.Size         = props.Size end
    if props.Position     then img.Position     = props.Position end
    if props.AnchorPoint  then img.AnchorPoint  = props.AnchorPoint end
    if props.LayoutOrder  then img.LayoutOrder  = props.LayoutOrder end
    if props.ZIndex       then img.ZIndex       = props.ZIndex end

    if type(resolved) == "table" then
        img.Image            = "rbxassetid://" .. tostring(resolved.id)
        img.ImageRectSize    = resolved.imageRectSize
        img.ImageRectOffset  = resolved.imageRectOffset
    else
        img.Image = tostring(resolved)
    end
...
local function padding(parent, p)
    ...
    return new("UIPadding", {
        PaddingTop    = UDim.new(0, p[1] or 0),
        PaddingRight  = UDim.new(0, p[2] or 0),
        PaddingBottom = UDim.new(0, p[3] or 0),
        PaddingLeft   = UDim.new(0, p[4] or 0),
        Parent = parent,
    })
end
...
-- Ripple effect: clicking spawns a circle that grows and fades.
local function attachRipple(button, color, transparency)
    color = color or Color3.fromRGB(255, 255, 255)
    button.ClipsDescendants = true
    button.MouseButton1Down:Connect(function(x, y)
        local circle = Instance.new("Frame")
        circle.AnchorPoint = Vector2.new(0.5, 0.5)
        local rel = Vector2.new(x - button.AbsolutePosition.X,
                                y - button.AbsolutePosition.Y)
        circle.Position = UDim2.fromOffset(rel.X, rel.Y)
        circle.Size = UDim2.fromOffset(0, 0)
        circle.BackgroundColor3 = color
        circle.BackgroundTransparency = transparency or 0.65
        circle.BorderSizePixel = 0
        circle.ZIndex = (button.ZIndex or 1) + 1
        local c = Instance.new("UICorner")
        c.CornerRadius = UDim.new(1, 0)
        c.Parent = circle
        circle.Parent = button
        local maxAxis = math.max(button.AbsoluteSize.X, button.AbsoluteSize.Y) * 2.2
        tweenInfo(circle, EASE.Slide(0.45), {
            Size = UDim2.fromOffset(maxAxis, maxAxis),
...
-- Shimmer: a light streak sweeping across a frame on a loop.
local function attachShimmer(frame, color, period)
    period = period or 4
    local streak = Instance.new("Frame")
    streak.BackgroundTransparency = 1
    streak.BorderSizePixel = 0
    streak.AnchorPoint = Vector2.new(0, 0)
    streak.Size = UDim2.new(0.4, 0, 1, 0)
    streak.Position = UDim2.fromScale(-0.5, 0)
    streak.ZIndex = (frame.ZIndex or 1) + 1
    local grad = Instance.new("UIGradient")
    grad.Color = ColorSequence.new(color or Color3.fromRGB(255,255,255))
    ...
    task.spawn(function()
        while streak.Parent do
            streak.Position = UDim2.fromScale(-0.5, 0)
            tweenInfo(streak, TweenInfo.new(period * 0.55,
...
-- Briefly pop scale on a frame that has AnchorPoint(0.5, 0.5) (or reasonable origin).
local function popScale(obj, factor, duration)
    factor = factor or 1.12
    duration = duration or 0.15
    local original = obj.Size
    local sx, sy = original.X, original.Y
    local target = UDim2.new(
        sx.Scale * factor, sx.Offset * factor,
        sy.Scale * factor, sy.Offset * factor)
    tweenInfo(obj, TweenInfo.new(duration, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = target})
    task.delay(duration, function()
        if obj.Parent then
            tweenInfo(obj, TweenInfo.new(duration, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = original})
        end
    end)
end
...
local function parentToHost(gui)
    -- Try CoreGui first (executor environment), fall back to PlayerGui.
...
local function makeScroll(parent, theme)
    local sf = new("ScrollingFrame", {
        ...
        ScrollBarImageColor3    = theme.InputBorder,
...
----------------------------------------------------------------
-- ELEMENT FACTORIES (used by Section)
----------------------------------------------------------------
local Element = {}

-- Common card frame for elements that have a Title/Desc on the left and a
...
local function titleBlock(parent, theme, title, desc)
    ...
    local lblTitle = new("TextLabel", {
        ...
        Size                   = desc and UDim2.new(1, 0, 0, 18) or UDim2.fromScale(1, 1),
        Position               = UDim2.new(),
        Parent = holder,
    })
    local lblDesc
    if desc and desc ~= "" then
        lblDesc = new("TextLabel", {
            BackgroundTransparency = 1,
            Font                   = FONT,
            Text                   = desc,
    ...
    return holder, lblTitle, lblDesc
end
...
function Element.Button(section, cfg)
    ...
    return { _card = card, _button = btn }
end

-- INPUT ----------------------------------------------------------------------
function Element.Input(section, cfg)
    local theme = section._window.Theme
    cfg = cfg or {}
    local isArea = (cfg.Type == "Textarea")
    local height = isArea and 110 or ((cfg.Desc and cfg.Desc ~= "") and 92 or 78)

    local card = elementCard(section._content, theme, height)

    local titleH = new("TextLabel", {
        BackgroundTransparency = 1,
        Font = FONT_MED, Text = cfg.Title or "Input",
    ...
    if cfg.Desc and cfg.Desc ~= "" then
        new("TextLabel", {
            ...
            TextColor3 = theme.TextLight, TextSize = 12,
            TextXAlignment = Enum.TextXAlignment.Left,
            TextWrapped = true,
    ...
    local api = { _card = card, _box = box }
    function api:SetValue(v) box.Text = tostring(v or "") end
    function api:Get() return box.Text end
    return api
end

-- SLIDER ---------------------------------------------------------------------
function Element.Slider(section, cfg)
    local theme = section._window.Theme
    cfg = cfg or {}
    local v = cfg.Value or {}
    local minV, maxV, def = v.Min or 0, v.Max or 100, v.Default or v.Min or 0
    local step = cfg.Step or 1
    local card = elementCard(section._content, theme, 64)
    ...
    local current = def
    local lastShown = def
    local function setValue(val, fire, animate)
        val = math.clamp(val, minV, maxV)
        if step and step > 0 then
            val = math.floor((val - minV) / step + 0.5) * step + minV
            val = math.clamp(val, minV, maxV)
        end
        current = val
    ...
    local function updateFromInput(input)
        ...
        setValue(minV + rel * (maxV - minV), true, false)
    end
    ...
    local api = { _card = card }
    function api:Set(val) setValue(val, true, true) end
    function api:Get() return current end
    return api
end

-- DROPDOWN -------------------------------------------------------------------
function Element.Dropdown(section, cfg)
    local theme = section._window.Theme
    cfg = cfg or {}
    ...
    if multi then
        ...
        if type(cfg.Value) == "table" then
            for _, v in ipairs(cfg.Value) do selected[v] = true end
        end
    else
        selected = cfg.Value or values[1]
    end

    local card = elementCard(section._content, theme,
        (cfg.Desc and cfg.Desc ~= "") and 64 or 50)

    titleBlock(card, theme, cfg.Title or "Dropdown", cfg.Desc)
    ...
    local function rebuild()
        ...
        popup.Size = UDim2.fromOffset(200, math.min(#values, 6) * 30 + 12)
    end
    ...
    local function openFn()
        ...
        popup.Size     = UDim2.fromOffset(sz.X, math.min(#values, 6) * 30 + 12)
        popupScale.Scale = 0.85
        popup.Visible  = true
        tween(chev, 0.2, {Rotation = 180})
        tweenInfo(popupScale, EASE.Spring(0.28), {Scale = 1})
        -- Stagger items in
        local i = 0
        for _, c in ipairs(popup:GetChildren()) do
            if c:IsA("TextButton") then
                ...
                task.delay(delay, function()
                    if c.Parent then
                        tween(c, 0.18, {BackgroundTransparency = 0})
                    end
                end)
                i = i + 1
            end
    ...
    UserInputService.InputBegan:Connect(function(input, gp)
        if popup.Visible and (input.UserInputType == Enum.UserInputType.MouseButton1
            ...
            local inPopup = mp.X >= pa.X and mp.X <= pa.X + ps.X
                        and mp.Y >= pa.Y and mp.Y <= pa.Y + ps.Y
            local inBtn   = mp.X >= ba.X and mp.X <= ba.X + bs.X
                        and mp.Y >= ba.Y and mp.Y <= ba.Y + bs.Y
            if not inPopup and not inBtn then closeFn() end
        end
    end)
...
-- PARAGRAPH ------------------------------------------------------------------
function Element.Paragraph(section, cfg)
    local theme = section._window.Theme
    cfg = cfg or {}
    local card = new("Frame", {
        BackgroundColor3 = theme.InputBg,
    ...
    local api = { _card = card }
    function api:SetDesc(text) lblDesc.Text = tostring(text or "") end
    function api:SetTitle(text) lblTitle.Text = tostring(text or "") end
    return api
end

-- LABEL ----------------------------------------------------------------------
function Element.Label(section, cfg)
    local theme = section._window.Theme
    cfg = cfg or {}
    local lbl = new("TextLabel", {
        BackgroundTransparency = 1,
        Font = FONT, Text = cfg.Title or cfg.Text or "",
        TextColor3 = theme.TextLight, TextSize = 13,
        TextXAlignment = Enum.TextXAlignment.Left,
        TextWrapped = true,
        AutomaticSize = Enum.AutomaticSize.Y,
        Size = UDim2.new(1, 0, 0, 18),
...
function Section.new(tab, cfg)
    ...
    local container = new("Frame", {
        ...
        AutomaticSize    = Enum.AutomaticSize.Y,
        LayoutOrder      = #tab._content:GetChildren(),
        Parent = tab._content,
    })
    ...
    if cfg.Title and cfg.Title ~= "" then
        new("TextLabel", {
            BackgroundTransparency = 1,
            Font = FONT_BOLD, Text = string.upper(cfg.Title),
    ...
    local content = new("Frame", {
        BackgroundTransparency = 1,
        Position = UDim2.new(0, 0, 0, (cfg.Title and cfg.Title ~= "") and 22 or 0),
        Size     = UDim2.new(1, 0, 0, 0),
        AutomaticSize = Enum.AutomaticSize.Y,
        Parent = container,
    })
...
function Section:Button(cfg)    return Element.Button(self, cfg)    end
function Section:Input(cfg)     return Element.Input(self, cfg)     end
function Section:Slider(cfg)    return Element.Slider(self, cfg)    end
function Section:Dropdown(cfg)  return Element.Dropdown(self, cfg)  end
function Section:Paragraph(cfg) return Element.Paragraph(self, cfg) end
function Section:Label(cfg)     return Element.Label(self, cfg)     end
...
function Tab.new(window, cfg)
    ...
    local title = new("TextLabel", {
        BackgroundTransparency = 1,
        Font = FONT_MED, Text = cfg.Title or "Tab",
        TextColor3 = theme.TextLight, TextSize = 13,
        TextXAlignment = Enum.TextXAlignment.Left,
    ...
    local page = new("Frame", {
        ...
        Size = UDim2.fromScale(1, 1),
        Visible = false,
        Parent = window._pageHost,
    })
    local scroll = makeScroll(page, theme)
    padding(scroll, {6, 14, 14, 14})
...
function Tab:Select()
    ...
    local function tintIcon(inst, color)
        if inst:IsA("ImageLabel") then
            tween(inst, 0.15, {ImageColor3 = color})
        else
            tween(inst, 0.15, {TextColor3 = color})
        end
    end
    -- Cross-fade out old page
    if window._activeTab then
        local prev = window._activeTab
        tween(prev._btn,       0.2, {BackgroundTransparency = 1})
        tween(prev._title,     0.2, {TextColor3 = theme.TextLight,
                                     Position = UDim2.new(0, 38, 0, 0)})
        tween(prev._indicator, 0.2, {Size = UDim2.fromOffset(3, 0)})
        tintIcon(prev._icon, theme.TextLight)
        -- Slight slide+fade for old page contents
        if prev._content then
            for _, child in ipairs(prev._content:GetChildren()) do
                if child:IsA("GuiObject") then
                    tween(child, 0.15, {Position = child.Position + UDim2.fromOffset(-8, 0)})
                end
            end
        end
        task.delay(0.16, function()
            if window._activeTab ~= prev then
                prev._page.Visible = false
                if prev._content then
                    for _, child in ipairs(prev._content:GetChildren()) do
                        if child:IsA("GuiObject") then
                            child.Position = child.Position + UDim2.fromOffset(8, 0)
                        end
                    end
                end
            end
    ...
    if window._headerTitle then
        window._headerTitle.Text = self._cfg.Title or ""
    end
end

function Tab:Section(cfg) return Section.new(self, cfg) end
...
function NimiUI:CreateWindow(cfg)
    ...
    local gui = new("ScreenGui", {
        Name = "NimiUI_" .. (cfg.Folder or "Window"),
    ...
    local tabScroll = makeScroll(tabScrollHolder, theme)
    ...
    local ver = new("TextLabel", {
        BackgroundTransparency = 1,
        Font = FONT, Text = (cfg.Folder or "NimiUI") .. " · v" .. NimiUI.Version,
    ...
    task.spawn(function()
        while header.Parent do
            tweenInfo(headerGrad, TweenInfo.new(8, Enum.EasingStyle.Sine,
                Enum.EasingDirection.InOut), { Rotation = 25 })
            task.wait(8)
            tweenInfo(headerGrad, TweenInfo.new(8, Enum.EasingStyle.Sine,
                Enum.EasingDirection.InOut), { Rotation = -25 })
            task.wait(8)
    ...
    -- Popup host (lives at root level so dropdowns float above)
    self._popupHost = new("Frame", {
        BackgroundTransparency = 1,
        Size = UDim2.fromScale(1, 1),
        ZIndex = 50,
        Parent = gui,
    ...
    -- Drag the window from the header
    makeDraggable(root, header)

    -- Drive open/close/minimize with a UIScale instead of mutating Size,
    -- so children don't reflow during the animation.
    ...
    -- Mount animation: spring scale up + content stagger
    rootScale.Scale = 0.6
    root.BackgroundTransparency = (cfg.Transparent and 0.1 or 0)
    tweenInfo(rootScale,
...
local function ensureNotifyGui()
    ...
    notifyGui = new("ScreenGui", {
        Name = "NimiUI_Notifications",
        ResetOnSpawn = false,
        IgnoreGuiInset = true,
        ZIndexBehavior = Enum.ZIndexBehavior.Sibling,
        DisplayOrder = 999,
    })
    parentToHost(notifyGui)
...
function NimiUI:Notify(cfg)
    ...
    tweenInfo(cardScale, EASE.Spring(0.4), { Scale = 1 })
    for _, c in ipairs(card:GetDescendants()) do
        if c:IsA("TextLabel") then
            tween(c, 0.3, {TextTransparency = 0})
        elseif c:IsA("ImageLabel") then
            tween(c, 0.3, {ImageTransparency = 0})
        end
    end

    local dur = tonumber(cfg.Duration) or 4
    if dur > 60 then dur = dur / 1000 end -- tolerate ms input
    -- Drain progress bar
    tweenInfo(progressFill,
    ...
    local function dismiss()
        ...
        for _, c in ipairs(card:GetDescendants()) do
            if c:IsA("TextLabel") then
                tween(c, 0.18, {TextTransparency = 1})
            elseif c:IsA("ImageLabel") then
                tween(c, 0.18, {ImageTransparency = 1})
            end
        end
        task.wait(0.26)
        card:Destroy()
    ...
    local clickArea = new("TextButton", {
        ...
        Size = UDim2.fromScale(1, 1), ZIndex = 100, Parent = card,
    })
...
-- THEME ACCESS / MISC
----------------------------------------------------------------
function NimiUI:SetTheme(name)
    -- Reserved: actual live retheming of an existing window is non-trivial
    -- because every element captures `theme` in closures. For now this just
    -- sets the default theme used by future windows.
